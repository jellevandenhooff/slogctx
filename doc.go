...
// useful to ensure a context is always included.
//
...
// This is useful to include a requestID with all logs.
//
//	ctx = slogctx.WithAttrs(ctx, "requestID", 1234)
//	slogctx.Info(ctx, "processing request")
//	slog.Default().WithContext(ctx).Info("processing more") // also works with plain *slog.Logger
//
...
// is useful to debug specific requests.
//
